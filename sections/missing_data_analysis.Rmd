...
```{r sprawdz_zatwierdzonych_bez_dat}
...
```{r analizuj_zatwierdzonych_bez_ceny}
...
dane_clean <- dane %>%
  ...
  # 5. Czyszczenie AMT_APPLICATION - imputacja z AMT_CREDIT jeśli dostępne
  mutate(
    AMT_APPLICATION = case_when(
      AMT_APPLICATION == 0 & AMT_CREDIT > 0 ~ AMT_CREDIT,      # imputacja z przyznanej kwoty
      TRUE ~ AMT_APPLICATION                                   # pozostaw bez zmian (w tym 0)
    )
  )
