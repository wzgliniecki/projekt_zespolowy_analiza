...
- Blok raty: AMT_ANNUITY i CNT_PAYMENT - dokładnie ta sama liczba braków (5589). To wskazuje, że brak raty (`AMT_ANNUITY`) idzie w parze z brakiem liczby płatności (`CNT_PAYMENT`).
...
Aby wykryć takie przypadki, przyjęto próg 50 lat (18250 dni). Jest to granica, która z jednej strony eliminuje wartości nierealistyczne, a z drugiej nie usuwa żadnych potencjalnie poprawnych obserwacji. Wszystkie rekordy przekraczające ten próg potraktowano jako błędne wartości systemowe i zaklasyfikowano jako braki danych.
...
W zmiennych typu `DAYS` zidentyfikowano liczne wartości przekraczające 50 lat, przy czym największa skala problemu dotyczy `DAYS_FIRST_DRAWING`, a pozostałe pola również zawierają od kilkuset do kilku tysięcy nienaturalnie dużych wartości, co jednoznacznie wskazuje na systemowy placeholder stosowany w miejsce rzeczywistych dat. W dalszej części analizy zweryfikowano statusy kontraktów zawierających te wartości, aby określić, czy są one logicznym skutkiem przebiegu procesu kredytowego, czy też stanowią błąd systemowy.
...
status_by_var_df %>%
  kbl(
    caption = "Statusy kontraktów dla fejkowych wartości w poszczególnych zmiennych DAYS*",
...
Aby ustalić, czy te kontrakty były faktycznie przygotowane do uruchomienia, konieczne jest ponowne sprawdzenie ich parametrów finansowych — przede wszystkim.
...
summary_cnt %>%
  kbl(
    caption = "Podsumowanie CNT_PAYMENT i AMT_CREDIT dla kontraktów z fałszywymi wartościami DAYS*",
    col.names = c("Zmienna", "Min", "Q1", "Mediana", "Średnia", "Q3", "Max")
  ) %>%
  kable_classic(full_width = FALSE, html_font = "Arial") %>%
  row_spec(0, bold = TRUE)
...
Jednak mniej typowa sytuacja dot. 632 zatwierdzonych kontraktów. One powininny mieć określoną cenę towaru. Sprawdzamy, czy mają inne kluczowe informacje (jak poprzednio: `AMT_CREDIT` i `CNT_PAYMENT`).
...
W większości przypadków są to kontrakty anulowane lub ddrzucone, gdzie brak kwoty wnioskowanej jest logiczny.
