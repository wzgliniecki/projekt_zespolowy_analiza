# Eksploracyjna analiza danych (EDA) w procesie oceny jakości i struktury zbioru
...
Takie zestawienie danych pozwala na przeprowadzenie wielowymiarowej analizy – zarówno [jakościowej]{.underline} (wzorce zachowań), jak i [ilościowej]{.underline} (wartości kredytowe).
...
# Metody czyszczenia, transformacji i przygotowania danych do analizy statystycznej

## Identyfikacja, charakterystyka i interpretacja brakujących danych (NA) w zbiorze
...
-   [Braki marginalne]{.underline}: `PRODUCT_COMBINATION` (0.02%).

-   [Średni poziom]{.underline}: Zmienne finansowe (`AMT_ANNUITY`, `CNT_PAYMENT`, `AMT_GOODS_PRICE`) oscylują wokół 22-23% braków.

-   [Wysoki poziom]{.underline}: Zmienne związane z harmonogramem spłat i ubezpieczeniem wykazują blisko 40% braków.

-   [Krytyczny poziom]{.underline}: Wkład własny przekracza 53%, a stopy procentowe są niemal całkowicie puste (99.69% braków).

### Wizualna analiza struktury i współwystępowania braków danych
...
### Analiza współzależności braków danych pomiędzy zmiennymi

#### Braki danych w zmiennych dotyczących rat kredytowych i ich implikacje procesowe
...
Zmienna `AMT_ANNUITY` reprezentuje [kwotę miesięcznej raty kredytowej]{.underline}, natomiast `CNT_PAYMENT` określa [liczbę rat]{.underline} przewidzianych w harmonogramie spłat dla danego wniosku. Analiza wykazała, że braki występują wyłącznie w kontraktach o statusie anulowanym, niewykorzystanym lub po odmowie. Wynika to z logiki biznesowej: dla wniosku, który nie został uruchomiony, nie wylicza się harmonogramu spłat. Brak danych jest więc informacją samą w sobie – "kontrakt nie powstał".

#### Braki danych w zmiennych harmonogramu kredytu i ich znaczenie dla interpretacji cyklu życia kontraktu
...
Zmienne z tej grupy opisują kluczowe momenty w harmonogramie kredytu oraz jedną cechę dotyczącą decyzji klienta przy składaniu wniosku. Określają one, [ile dni]{.underline} upływa od daty aplikacji do poszczególnych etapów cyklu życia kontraktu.
...
#### Nieobowiązkowy charakter wkładu własnego jako źródło braków danych
...
#### Ocena przydatności zmiennych dotyczących oprocentowania w kontekście niemal całkowitego braku danych
...
#### Incydentalne braki w zmiennej opisującej kombinacje produktów kredytowych
...
#### Analiza braków danych dotyczących wartości towaru i ich związek ze statusem kontraktu
...
#### Interpretacja braków informacji o osobie towarzyszącej klientowi podczas składania wniosku
...
#### Wartości nietypowe i ukryte braki danych w zmiennych finansowych
...
### Strategia czyszczenia danych oparta na logice procesów kredytowych i imputacji biznesowej

Na podstawie przeprowadzonej analizy brakujących wartości (EDA), stwierdzono, że występowanie NA w zbiorze [nie jest przypadkowe]{.underline}, lecz ściśle powiązane z logiką procesów kredytowych. Większość braków w kluczowych zmiennych finansowych i harmonogramowych występuje przy wnioskach o statusie *anulowanym*, *odmówionym* oraz przy *niewykorzystanych ofertach*. Oznacza to, że parametry takie jak rata kredytu, liczba płatności czy dokładne daty spłat fizycznie nie istnieją dla kontraktów, które nigdy nie weszły w fazę realizacji.
...
#### Implementacja reguł imputacji wynikających z analizy procesowej i struktury braków danych
