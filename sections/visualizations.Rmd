...
2. Kwota kredytu vs. status kontraktu. Czy większe kwoty częściej kończą się odmową?

```{r wykres_kwota_wg_statusu, warning=FALSE}
...
3. Typ klienta vs. ryzyko. Czy nowi klienci częściej mają problemy ze spłatą?
...
4. Facetowanie wg dnia tygodnia. Czy dzień złożenia wniosku ma wpływ na zatwierdzenie?

```{r wykres_dzien_tygodnia_kwoty, warning=FALSE}
...
5. Liczba rat vs. status kontraktu. Czy długość spłaty wpływa na ryzyko?
...
if (is_html_output) {
  ...
  p <- plot_ly(dane_clean, x = ~CNT_PAYMENT, y = ~AMT_CREDIT,
          color = ~NAME_CONTRACT_STATUS_PL, colors = status_colors, type = "scatter", mode = "markers") %>%
    layout(
...
9. Status kontraktu vs. branża sprzedawcy. Czy branża wpływa na decyzję kredytową?

```{r wykres_branza_sprzedawcy}
...
10. Status kontraktu vs. kanał pozyskania klienta. Czy kanał sprzedaży wpływa na decyzję kredytową?


```{r wykres_kanal_sprzedazy}}
...
11. Najczęstsze powody odmowy aplikacji

```{r wykres_powody_odmow}}
dane_clean %>%
...
12. Status kontraktu vs. typ umowy. Czy rodzaj umowy wpływa na decyzję kredytową?
...
13. Godzina aplikacji vs. status kontraktu. Czy godzina złożenia wniosku ma wpływ na decyzję kredytową?

```{r wykres_godzina_aplikacji}
...
14. Cena towaru vs. status kontraktu i ubezpieczenie. Czy ubezpieczenie wpływa na cenę towaru?
...
15. Kwota wnioskowana vs. typ klienta i kanał pozyskania. Czy typ klienta i kanał wpływają na kwotę wnioskowaną?

```{r wykres_kwota_typ_klienta_kanal}
