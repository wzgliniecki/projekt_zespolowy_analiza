...
## Statystyki opisowe kluczowych zmiennych numerycznych
...
## Standaryzacja parametrów kredytowych i analiza odchyleń od wartości przeciętnych
...
Podsumowując to jednym zdaniem - [im wniosek kredytowy był bliższy przeciętnemu tym większa była szansa na jego akceptację]{.underline}.

## Ocena zależności pomiędzy zmiennymi kategorycznymi a statusem kontraktu z wykorzystaniem testu chi‑kwadrat

W tej części analizie poddano zależności pomiędzy [wybranymi zmiennymi kategorycznymi a statusem kontraktu kredytowego]{.underline}. Do oceny istotności statystycznej wykorzystano test **chi‑kwadrat** niezależności, który pozwala sprawdzić, czy rozkład kategorii danej zmiennej różni się pomiędzy grupami statusów.

Test chi‑kwadrat porównuje liczebności [obserwowane]{.underline} (rzeczywiste) z liczebnościami [oczekiwanymi]{.underline}, które wystąpiłyby, gdyby zmienne były od siebie niezależne. Jeżeli różnice są duże, statystyka Chi² [rośnie]{.underline}, a wartość p spada — co wskazuje na istotną zależność.
...
## Analiza różnic w wysokości przyznanego finansowania w zależności od statusu kontraktu
...
-   Istnieją [istotne]{.underline} statystycznie różnice w wysokości przyznanych kwot kredytu między grupami kontraktowymi (Welch ANOVA: p \< 2.23e−308, ω² = 0.61) — efekt jest bardzo silny.
...
-   Wysoka kwota kredytu [nie gwarantuje]{.underline} zatwierdzenia, ale jest [silnie związana]{.underline} z decyzją końcową — może być traktowana jako wskaźnik ryzyka.
...
## Wpływ typu klienta na wynik procesu kredytowego – analiza statystyczna

Każdy słupek reprezentuje jedną kategorię statusu, a kolory wskazują typ klienta. Dodatkowo wykres zawiera wyniki testu *chi‑kwadra*t oraz miarę siły zależności (*Cramér’s V*), co pozwala ocenić istotność statystyczną obserwowanych różnic.
...
-   Zależność między typem klienta a statusem kontraktu jest [istotna]{.underline} statystycznie (χ² = 2770.34, p \< 2.23e−308, Cramer's V = 0.19) — czyli umiarkowana siła powiązania.
...
## Porównanie długości harmonogramu spłaty w grupach statusów kontraktu
...
-   Różnice w liczbie rat między grupami kontraktowymi są bardzo silnie [istotne]{.underline} statystycznie (Welch ANOVA: p \< 2.23e−308, ω² = 0.79) — efekt jest ekstremalnie silny.
...
## Analiza zależności pomiędzy kwotą wnioskowaną a przyznaną w procesie kredytowym

Wykres zawiera linię regresji oraz histogramy brzegowe, co pozwala jednocześnie ocenić [siłę korelacji]{.underline} i [rozkład]{.underline} obu zmiennych. Statystyki testowe wskazują na związek między tymi wartościami.
...
## Charakterystyka liczby rat wśród zatwierdzonych kontraktów kredytowych

Histogram pokazuje zarówno [częstość występowania]{.underline} poszczególnych wartości, jak i [statystyki opisowe]{.underline} oraz wynik testu [t‑Studenta]{.underline}, co pozwala ocenić, jak typowa jest liczba rat w tej grupie klientów.
...
-   Średnia liczba rat wynosi 14.08, co wskazuje na preferencję dla kredytów [średnioterminowych]{.underline} — ani bardzo krótkich, ani długoterminowych.

-   Rozkład jest [skośny]{.underline}, z wyraźnym szczytem w przedziale 6–20 rat — najczęściej wybierane są kredyty rozłożone na 12–18 miesięcy.
...
## Struktura typów umów kredytowych w populacji oraz ich powiązanie ze statusem kontraktu

Dwa wykresy kołowe pokazują [udział poszczególnych statusów]{.underline} w każdej grupie, a dodatkowe statystyki (*test chi‑kwadrat, Cramér’s V*) pozwalają ocenić siłę i istotność zależności między typem klienta a statusem kontraktu.
...
-   Różnice są istotne statystycznie (*χ² = 2593.64, p \< 2.23e−308, Cramer's V = 0.34*) — czyli typ klienta ma [wyraźny wpływ]{.underline} na decyzję kredytową.
...
-   Wkład własny (`AMT_DOWN_PAYMENT`) ma istotny wpływ na kwotę kredytu\
    (*β̂ = 0.23, p = 0.03*) — [im wyższy wkład własny, tym wyższa przyznana kwota]{.underline}. To zgodne z logiką ryzyka: klient angażujący własne środki otrzymuje większe finansowanie.
...
## Analiza wariancji parametrów kredytowych w zależności od statusu kontraktu (Welch ANOVA)
...
### Różnice w kwocie wnioskowanej pomiędzy grupami statusów kontraktu (Welch ANOVA)
...
-   Różnice między grupami są ekstremalnie [istotne]{.underline} statystycznie, a efekt wielkości *𝜔2=0.82* — czyli bardzo silny.
...
Wszystkie porównania parowe są [istotne]{.underline} (*p_Holm-adj \< 1e−7*), co potwierdza, że każda grupa różni się od pozostałych.

## Analiza wariancji liczby rat w zależności od statusu kontraktu (Welch ANOVA)
...
-   Tu również różnice między grupami są ekstremalnie [istotne]{.underline} statystycznie\
...
## Porównanie wysokości rat kredytowych pomiędzy grupami statusów kontraktu (Welch ANOVA)
...
-   Trzeci wykres także pokazuje [istotne]{.underline} różnice między grupami. Test Welch’a wykazał bardzo silny efekt, a efekt również wielkości *𝜔2= 0.79*.
...
## Syntetyczne podsumowanie wyników analizy statystycznej parametrów kredytowych
...
To oznacza, że [system preferuje wnioski bez skrajności]{.underline} — ani bardzo wysokich kwot, ani bardzo długich harmonogramów, ani nietypowych parametrów.
...
## Implikacje analityczne i rekomendacje strategiczne wynikające z analizy statystycznej
